...
class AppStore extends Component {
  ...
  getFilteredAppItem = () => {
    ...
    const filteredAppItem = appsList.filter(
      eachAppDetails => eachAppDetails.category === ActiveTabId,
    )
    return filteredAppItem
  }

  render() {
    const {ActiveTabId, searchInput} = this.state
    const filteredAppItem = this.getFilteredAppItem(ActiveTabId)
    const searchResult = filteredAppItem.filter(each =>
